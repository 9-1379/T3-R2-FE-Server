  ...
  <div>
    ...
    <div class="badge-list">
      ...
      <div class="badge-header">
        <button class="add-badge-btn" @click="changePopState()">
          배지 추가
        </button>
      </div>
      <table>
  ...
  methods: {
    async fetchBadges() {
      try {
        ...
        this.badges = response.data.filter(
          (badge) => badge.status === "Enabled"
        );
      } catch (error) {
    ...
    deactivateBadge(badgeName) {
      axiosInstance
        .post("/api/admin/badge/deactivate", null, { params: { badgeName } })
        .then(() => {
