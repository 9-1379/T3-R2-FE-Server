...
methods: {
  get() {
    this.axiosInstance.get('/employees/dashboard/:id').then((response) => {
  ...
  async fetchEmployee(employeeId) {
    try {
      const response = await axiosInstance.get(`/employees/dashboard/${employeeId}`);
...
deleteIntroduction() {
  const employeeId = this.$route.params.id;
    if (employeeId) {
      try {
        axiosInstance.delete(`/employees/dashboard/${employeeId}/introduction`);
